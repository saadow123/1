...
from lms.djangoapps.courseware.permissions import CALENDAR_SYNC
...
class CalendarSyncToggleTool(CourseTool):
    ...
    def is_enabled(cls, request, course_key):
        ...
        """
        if CALENDAR_SYNC_FLAG.is_enabled(course_key) and RELATIVE_DATES_FLAG.is_enabled(course_key):
            if (request.user.has_perm(CALENDAR_SYNC, course_key) or
